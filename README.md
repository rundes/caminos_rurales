...
- **Los kilómetros nunca cruzan una pausa ni una interrupción.** `kmDeTrack` (`lib/track.ts`) suma distancia dentro de cada segmento del track, nunca entre el último punto de antes de un corte y el primero de después — ni en el estimado local que se muestra antes de subir (`lib/local/cierre.ts`) ni en el que calcula y persiste el servidor (`app/dashboard/recorrido/actions.ts`, columna `recorridos.km`). El servidor no confía en huecos que declare el cliente: deriva los cortes él mismo, de la **unión** de dos señales independientes (`unionCortes`, `lib/track.ts`) — con cualquiera de las dos alcanza para cortar:
  - **por tiempo** (`derivarCortes`), con el mismo umbral del watchdog (30 s, `UMBRAL_INTERRUPCION_MS`) sobre los timestamps de los puntos GPS crudos (`datos.puntos`), y sólo cuando ese array viene alineado índice a índice con el track subido (misma longitud);
  - **por distancia** (`derivarCortesPorDistancia`), directo sobre la geometría del track (5 km, `UMBRAL_INTERRUPCION_DISTANCIA_M`), sin depender en absoluto de `puntos`. Es la que sigue cortando un salto aunque `puntos` venga recortado, desalineado o ausente — el caso en que la señal de tiempo no aporta nada, así que sin esta segunda señal un payload armado a mano podría volver a acreditar como recorrido un tramo nunca relevado.
  Ni ocultar una pausa ni inventar una, ni forzar el desalineo de `puntos`, alcanza para inflar o recortar los km acreditados — y esos km son los que puntúan (`lib/juego.ts`) y los que evalúa la plausibilidad del recorrido. Los km con datos de sensores (`kmConSensores`, `lib/juego.ts`, y el desglose por calidad `kmPorCalidad`, `lib/recorrido-sensores-servidor.ts`) usan los mismos cortes, derivados de las muestras de sensores (que traen su propio timestamp) con `derivarCortesDeMuestras`: tampoco puentean una pausa.

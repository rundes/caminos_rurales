...
- [ ] Shapefiles/capas UBA (ver fase 6). Edición de geometrías de tramos: implementada en fase 14.K (dibujo manual click-to-add-vertex); pendiente sigue siendo importar/editar geometrías desde un shapefile.
...
- [x] K. Alta y edición de tramos (migración `0011_alta_tramos.sql`, `feat(tramos)`): políticas `tramos_insert_gestion`/`tramos_update_gestion` (`rol_actual() in ('municipio', 'auditor')` + `municipio = municipio_actual()`, en `using` y `with check`, mismo patrón que `caminos_insert`/`caminos_update` de 0001); sin política de delete — columna `activo` en su lugar, con la semántica documentada en la propia migración: el denominador de `cobertura_municipio`, la lista `/dashboard/tramos` y la capa de tramos del mapa cuentan/muestran solo tramos activos, pero `rugosidad_tramos`/`cuadros_por_tramo` y el detalle de un tramo (`/dashboard/tramos/<id>`, con etiqueta "Inactivo") nunca ocultan el historial ya registrado. Columnas de auditoría `creado_por`/`actualizado_at` selladas por el trigger `tramos_auditoria` (mirror de `fallas_estado_no_escalar`, 0010). UI: `/dashboard/tramos/nuevo` y `/dashboard/tramos/[id]/editar` (`TramoForm.tsx`, gateadas a municipio/auditor con `notFound()`), dibujo de la geometría click-to-add-vertex sobre Leaflet (`components/MapaDibujarTramo.tsx`, sin biblioteca de dibujo) con el km calculado en vivo. Server Actions `crearTramo`/`actualizarTramo` (`app/dashboard/tramos/actions.ts`): `km` se calcula siempre en el servidor a partir de la geometría (`kmDeGeometria`, `lib/tramos.ts`, reusa `kmDeTrack` de `lib/track.ts`) — el cliente nunca lo manda. `scripts/smoke.mjs` sección 13 cubre RLS (productor bloqueado, clave secreta puede, municipio no puede insertar en otro municipio) y la semántica de `activo` sobre el denominador de cobertura.
